...
- OPTEX\_INVOKED\_AS
    ...
    **optex**를 호출하는 데 사용된 원래 명령어 경로를 지정하십시오. 이는 **optex**가 래퍼 스크립트를 통해 호출되어 원래 `$0`이 손실된 경우에 유용합니다. 래퍼에 대한 심볼릭 링크가 실행될 때, 래퍼는 이 변수를 자체 `$0`으로 설정해야 합니다. 그래야 **optex**가 올바른 명령어 이름을 확인할 수 있습니다.
