...
- OPTEX\_INVOKED\_AS
    ...
    **optex** を呼び出すために使用された元のコマンドパスを指定します。これは、ラッパースクリプト経由で **optex** が呼び出され、元の `$0` が失われた場合に有用です。ラッパーへのシンボリックリンクが実行された場合、ラッパーは自分自身の `$0` をこの変数に設定し、**optex** が正しいコマンド名を判別できるようにする必要があります。
