...
模块还可以使用 C<__DATA__> 部分的 C<builtin> 指令定义内置选项。内置选项由 L<Getopt::Long> 处理，必须在目标命令名之前指定。例如

    optex -Mxform --xform-visible=2 cat file

这里的 C<--xform-visible> 是 C<xform> 模块中定义的内置选项。
...
在应用主配置之前加载额外的 TOML 片段。接受字符串或数组。每个条目可以是字面路径或 glob 模式。Tilde 会被展开，相对路径会根据声明包含的文件进行解析。包含是深度优先处理的；哈希值递归合并，数组追加，标量使用后进先出语义。主 F<config.toml> 最后应用，因此可以覆盖包含值。
...
指定创建符号链接时使用的路径（C<--ln>）。若未设置，则使用C<$0>。当通过封装脚本（如Homebrew安装）调用B<optex>时，此选项尤为有用——此时可让符号链接指向封装脚本而非实际脚本。

=item OPTEX_INVOKED_AS

指定用于调用 B<optex> 的原始命令路径。当通过封装脚本调用 B<optex> 且原始 C<$0> 丢失时，此设置尤为重要。执行指向封装脚本的符号链接时，该脚本应将此变量设置为自身的 C<$0>，以便 B<optex> 能够确定正确的命令名称。
