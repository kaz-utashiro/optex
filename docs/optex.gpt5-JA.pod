...
C<--ln> でシンボリックリンクを作成する際に使用するパスを指定します。未設定の場合は C<$0> が使用されます。これは、B<optex> がラッパースクリプト（例: Homebrew インストール）経由で呼び出され、シンボリックリンクを実際のスクリプトではなくラッパーを指すようにしたい場合に役立ちます。

=item OPTEX_INVOKED_AS

B<optex> を呼び出すために使用された元のコマンドパスを指定します。これは、ラッパースクリプト経由で B<optex> が呼び出され、元の C<$0> が失われた場合に有用です。ラッパーへのシンボリックリンクが実行された場合、ラッパーは自分自身の C<$0> をこの変数に設定し、B<optex> が正しいコマンド名を判別できるようにする必要があります。
